...
#
# log n 是指每次將數組分割為兩部分的過程，也就是遞歸劃分數組的深度。
# 快速排序每次選擇一個主元素（pivot）來將數組分成兩個子數組，然後分別對這兩個子數組進行排序。
# 在最壞情況下，如果每次選擇的主元素都不平衡，分割的深度將達到 n 層，因此快速排序的時間複雜度將退化到 O(n^2)。
#
# n 是指每次分割數組需要遍歷的元素的數量，也就是進行分割操作的時間複雜度。
# 快速排序通常選擇第一個元素或隨機選擇一個元素作為主元素，然後通過比較與主元素的大小關係，
# 將數組分為兩部分。因此，時間複雜度為 O(n)。
